...
export default class rkamyp2 extends EventEmitter {
  constructor(serializedState, view, settings, isDark) {
    ...
    atom.themes.onDidChangeActiveThemes(() => {
      _this.checkTheme();
      if (_this.isDark) {
        $('#rkamyp2').removeClass('light');
        $('.rkamyp2-status-bar').removeClass('light');
        $('.rkamyp2-logo').attr(
          'src',
        ...
        $('#rkamyp2').addClass('light');
        $('.rkamyp2-status-bar').addClass('light');
  ...
  setEventListeners() {
    ...
    this.settings.on('format_error', () => {
      // _this.terminal.writeln("JSON format error in rkamyp2.conf file") //TODO change to toast / error popup?
