...
export default class ApiWrapper {
  constructor(settings) {
    atom.config.observe('rkamyp.font_size', newValue => {
      try {
        ...
        atom.config.set(`rkamyp.font_size`, 14);
      }
    });
  }

  config(key) {
    return atom.config.get(`rkamyp.${key}`);
  }

  setConfig(key, value) {
    return atom.config.set(`rkamyp.${key}`, value);
  }

  openSettings() {
    atom.workspace.open('atom://config/packages/rkamyp');
  }
  ...
  onConfigChange(key, cb) {
    atom.config.onDidChange(`rkamyp.${key}`, cb);
  }
  ...
  getPackagePath() {
    return atom.packages.resolvePackagePath('rkamyp');
  }
  ...
  selectFirstProject() {
    if ($('.rkamyp-project') && $('.rkamyp-project').toArray()[0]) {
      $('.rkamyp-project')
        .toArray()[0]
        .click();
      const projectsButton = $('#rkamyp-projects');
  ...
  setFolder(name) {
    ...
    $('#rkamyp-projects #project-name').html(
      `<span class='project-name'>${projectName}</span>`,
    );
    $(document).ready(() => {
      const width = parseInt($('#rkamyp-options-left').width());
      $('#rkamyp-connection-tabs').css('left', width + 5);
    });
  }
