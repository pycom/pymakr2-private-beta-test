...
export default class PanelView extends EventEmitter {
  ...
  build() {
    ...
    this.resizer = $('#rkamyp-resizer');
    this.overlay_contents = $('#rkamyp-overlay-contents');
    this.topbar = $('#pycom-top-bar');
    this.title = $('#rkamyp-title');
    this.projects_button = $('#rkamyp-projects');
    this.projects_list = $('#rkamyp-projects .subnav');
    this.project_name_display = $('#rkamyp-projects #project-name');
    this.project_name = '';
    this.buttons = $('#rkamyp-buttons');
    this.button_more_tab = $('#rkamyp-more-subnav');
    this.overlay_wrapper = $('#rkamyp-overlay');
    this.terminal_area = $('#rkamyp-terminal-area');
    this.terminal_placeholder = $('#rkamyp-terminal-placeholder');
    this.button_more = $('#rkamyp-buttons #more');
    this.button_more_sub = $('#rkamyp-buttons #more .subnav');
    this.button_close = $('#rkamyp-buttons #close');
    this.comport_list = $('#rkamyp-comports-list');
    this.address_list = $('#rkamyp-address-list');
    this.device_connection_tabs = $('#rkamyp-connection-tabs');
    this.connect_all = $('#rkamyp-connect-all');
    this.close_all = $('#rkamyp-close-all');
    this.add_address_button = $('#rkamyp-add-address-button');
    this.add_address_field = $('#rkamyp-add-address-field');
    this.add_address_field_wrapper = $(
      '#rkamyp-add-address-field-wrapper',
    );
  ...
  showFeedbackPopup() {
    ...
    if (!this.feedback_popup_seen) {
      ...
      this.feedback_question.classList.add('rkamyp-feedback');
      this.feedback_question.innerHTML = '<h2>Hi rkamyp User!</h2> ';
      this.feedback_question.innerHTML +=
        'We are working on ideas for rkamyp 2.0 and would love your feedback! ';
  ...
  buildTerminal() {
    ...
    erd.listenTo(document.getElementById('rkamyp'), () => {
      if (_this.visible) {
  ...
  initResize(resizer) {
    ...
    erdUltraFast.listenTo(
      document.getElementById('rkamyp'),
      element => {
  ...
  createButton(id, icon, text, className, parent) {
    ...
    if (className && className !== '') {
      button.addClass(`rkamyp-${className}`);
    }
  ...
  setProjectName(name) {
    ...
    $(document).ready(() => {
      const width = parseInt($('#rkamyp-options-left').width());
  ...
  addComport(com_info) {
    ...
    $('#rkamyp-comports-list .loading_text').remove();
  ...
  addAddress(address) {
    ...
    $('#rkamyp-address-list .loading_text').remove();
    button.click(function() {
      _this.emit('connect.device', $(this).attr('name'));
    });
  }
  ...
  addConnectionTab(address) {
    ...
    $('#rkamyp-terminal-placeholder').css(
      'font-size',
      `${fontSize}px`,
    );
  ...
  removeConnectionTab(address) {
    ...
    if ($('#rkamyp-terminal-area div.device-terminal').length === 0) {
      this.terminal_placeholder.addClass('open');
      const runButton = $('#rkamyp-action-connect');
  ...
  openInfoOverlay(info) {
    this.overlay_wrapper.addClass('rkamyp-open');
    this.overlay_contents.addClass('rkamyp-open');
    this.overlay.open(info);
    $('.xterm-rows').addClass('blur-text');
  }

  closeOverlay() {
    this.overlay_wrapper.removeClass('rkamyp-open');
    this.overlay_contents.removeClass('rkamyp-open');
