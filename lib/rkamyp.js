...
export default class rkamyp extends EventEmitter {
  constructor(serializedState, view, settings, isDark) {
    ...
    atom.themes.onDidChangeActiveThemes(() => {
      _this.checkTheme();
      if (_this.isDark) {
        $('#rkamyp').removeClass('light');
        $('.rkamyp-status-bar').removeClass('light');
        $('.rkamyp-logo').attr(
          'src',
        ...
        $('#rkamyp').addClass('light');
        $('.rkamyp-status-bar').addClass('light');
  ...
  setEventListeners() {
    ...
    this.settings.on('format_error', () => {
      // _this.terminal.writeln("JSON format error in rkamyp.conf file") //TODO change to toast / error popup?
